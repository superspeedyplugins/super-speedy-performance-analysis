...
class SSPA_Traffic_Hot_Path {
    ...
    public static function boot($config) {
        // Old generated files load this current class after an upgrade. Their missing
        // generation cannot grant observation, and their row must not remain "running".
        if (empty($config['generation']) && !empty($config['collection_id'])) {
            add_action('plugins_loaded', static function() use ($config) {
                if (class_exists('SSPA_Traffic_Collection')) {
                    SSPA_Traffic_Collection::retire_legacy_observer((int) $config['collection_id']);
                }
            });
        }
        // Also protects observers generated by an older plugin template.
        if (!SSPA_Traffic_Authority::observe($config, static function() { return true; })) {
            return;
        }
    ...
    public static function flush() {
        return SSPA_Traffic_Authority::observe(self::$config, static function() {
            self::flush_authorized();
            return true;
        });
    }

    private static function flush_authorized() {
        if (!self::$config) {
            return;
        }
