...
// CLI and PHP-FPM have separate OPcache instances. After replacing generated PHP,
// wait for the documented two-second FPM revalidation interval before HTTP reads.
...
$after_rows=SSPA_Ajax_Profile::rows($after);
sspa_boundary_check(count($after_rows)===1&&$after_rows[0]['http_method']==='GET','method comparison includes one actual GET request in its own window');
