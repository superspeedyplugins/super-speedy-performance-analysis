...
add_action('template_redirect', static function () {
    ...
    $wpdb->get_var("SELECT option_value FROM {$wpdb->options} WHERE option_name = 'sspa_home_query_fixture_option'");
    $wpdb->get_var("SELECT COUNT(*) FROM {$wpdb->posts} WHERE post_title LIKE 'sspa_home_query_fixture%'");
    $wpdb->get_col("SELECT ID FROM {$wpdb->posts} WHERE post_type = 'sspa_home_query_fixture' LIMIT 1");
    $wpdb->get_var("SELECT 'sspa_home_query_fixture' AS marker FROM {$wpdb->users} LIMIT 1");
}, 5);
